...
class ParseBAM:
	...
	def readsNVC(self,outfile=None,nx=True, q_cut = 30, sample=""):
		'''for each read, calculate nucleotide frequency vs position'''
		if outfile is None:
			outfile1 = self.fileName + ".NVC.xls"
			outfile2 = self.fileName +".NVC_plot.r"
		...
		if nx:
			...
			print >>RS, 'plot(position,A_count/total,type="o",pch=20,ylim=c(0,1),col="dark green",xlab="Position of Read",ylab="Nucleotide Frequency",main="%s")' % sample
			print >>RS, 'lines(position,T_count/total,type="o",pch=20,col="red")'
			print >>RS, 'lines(position,G_count/total,type="o",pch=20,col="blue")'
			print >>RS, 'lines(position,C_count/total,type="o",pch=20,col="cyan")'
			...
			print >>RS, "total= A_count + C_count + G_count + T_count + N_count"
			print >>RS, "ym=max(A_count/total,C_count/total,G_count/total,T_count/total,N_count/total) + 0.05"
			print >>RS, "yn=min(A_count/total,C_count/total,G_count/total,T_count/total,N_count/total)"
			...
			print >>RS, 'lines(position,C_count/total,type="o",pch=20,col="cyan")'
			print >>RS, 'lines(position,N_count/total,type="o",pch=20,col="black")'		
			print >>RS, 'legend('+ str(len(RNA_read)-10) + ',ym,legend=c("A","T","G","C", "N"),col=c("dark green","red","blue","cyan","black"),lwd=2,pch=20,text.col=c("dark green","red","blue","cyan","black"))'
			print >>RS, "dev.off()"
	...
	def mRNA_inner_distance(self,outfile,refbed,low_bound=0,up_bound=1000,step=10,sample_size=1000000, q_cut=30):
					...
					if chrom in exon_bitsets:
						...
						if size == inner_distance:
							...
							if same_chrom:
								...
								FO.write(aligned_read.qname + '\t' + 'NA' + '\tPE_within_diff_chrom\n')
						elif size == 1:
		...
		print >>sys.stderr, "Total paired read used " + str(pair_num)
		if pair_num==0:
			print >>sys.stderr, "Cannot find paired reads"
			sys.exit(0)
