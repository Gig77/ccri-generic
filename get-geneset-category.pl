...
my $outfile = "/mnt/projects/generic/data/ccri/geneset_annotation.tsv";
...
open(IN, "/mnt/projects/helena_veronika/data/gsea_custom_gene_sets.gmt") or die "ERROR: Could not read input.\n";
while (<IN>) {
	my ($gs) = split /\t/;
...
open(IN, "/mnt/projects/iamp/results/gsea/custom/mir_target_genesets.gmx") or die "ERROR: Could not read input.\n";
while (<IN>) {
	my ($gs) = split /\t/;
	print OUT uc($gs),"\tCUSTOM\t\t\n";
}
...
open(IN, "/mnt/projects/generic/data/ccri/ccri_custom_gene_sets.gmt") or die "ERROR: Could not read input.\n";
while (<IN>) {
	my ($gs) = split /\t/;
	print OUT uc($gs),"\tCUSTOM\t\t\n";
}
